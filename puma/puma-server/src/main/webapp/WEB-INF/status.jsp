...
caption {
	padding: 0 0 5px 0;
	width: 700px;
	font: bold italic 15px "Trebuchet MS", Verdana, Arial, Helvetica,
		sans-serif;
	margin: 0 auto;
}
...
<table class="mytable">
	...
	<tr>
		...
		<th>Binlog</th>
		<th>Pos</th>
		<th>Pasred rows insert(since start)</th>
		<th>Parsed rows delete(since start)</th>
		<th>Parsed rows update(since start)</th>
		<th>Parsed ddl events(since start)</th>
	</tr>
	<%
	    Map<String, ServerStatus> servers = systemStatus.listServerStatus(); 
			Map<String, AtomicLong> updateCounter = systemStatus.listServerRowUpdateCounters();
			Map<String, AtomicLong> deleteCounter = systemStatus.listServerRowDeleteCounters();
			Map<String, AtomicLong> insertCounter = systemStatus.listServerRowInsertCounters();
			Map<String, AtomicLong> ddlCounter = systemStatus.listServerDdlCounters();
	...
	<%
	    if(ss.getDb() == null || ss.getDb().length()==0){
	%>
	All
	<%
	    }else{
...
<table class="mytable">
	...
	<%
	    Map<String, Long> storages = systemStatus.listStorageStatus(); 
...
<table class="mytable">
	...
	<tr>
		...
		<th>NeedDdl</th>
		<th>NeedDml</th>
		<th>NeedTransactionInfo</th>
	</tr>
	<%
	    Map<String, ClientStatus> clients = systemStatus.listClientStatus(); 
